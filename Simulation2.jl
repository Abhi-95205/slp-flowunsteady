...
run_name        = "wing simulation"       # Name of this simulation
save_path       = "/home/ubuntu/abhi_btech/test/rotorh"                  # Where to save this simulation
...
# Vehicle assembly
AOAwing         = 5.0                       # (deg) wing angle of attack
spanpos         = [-0.5, 0.5]               # Semi-span position of each rotor, 2*y/b
xpos            = [ 1 , 1 ]              	# x-position of rotors relative to LE, x/c
...
# Vehicle motion
magVvehicle     = 52.77                      # (m/s) vehicle velocity
...
speedofsound    = 340.3                    # (m/s) speed of sound
...
println("""
    ...
    ReD:    $(round(ReD, digits=3))
    Rec:    $(round(Rec, digits=3))
    Qinf: $(round(qinf, digits = 3))
""")
...
nsteps_per_rev  = 18                        # Time steps per revolution
...
angle_rotors = (-10*pi/180,20*pi/180,10*pi/180)
